...
const ImportWallet = () => {
	...
	const privateKeySet = (val: string) => {
		val.length == 0 ? setPrivateStatus(1) : val.length > 11 ? setPrivateStatus(1) : setPrivateStatus(0); 
		setPrivate(val);
	}
		...
		<Layout>
			<div style={{marginTop:'0'}}>
				<div className="card">
                    ...
                    <input type="password" style={{fontSize: "1em"}} onChange={(e) => privateKeySet(e.target.value)}  className='input-password w100' placeholder='Enter your Secret Recovery Phrase' />
					{
						privateStatus === 0 && privateKey != "" ?
						(
							<span style={{ color: "#f44336", fontSize: "0.75em" }}>Secret Recovery Phrases contain 12 words</span>
						) :
						""
					}
					<p className='p'>New password (min 8 chars)</p>
                    <input type="password" onChange={(e) => validatePass(e.target.value)} className='input-password w100'/>
					{
                    ...
                    <input type="password" className='input-password w100' onChange={(e) => matchPass(e.target.value)} />
					{
